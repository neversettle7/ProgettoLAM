				...
				<div id="container">
					<div id="content">
						...
						<p>
							...
							// Otteniamo la quantitÃ  di oggetti ordinati in caso di update
							//$qty = $_GET['qty'];
								...
								case 'remove' :
									...
									if ($cart) {
										...
										foreach ($items as $item) {
											// Cancelliamo l'elemento se il suo id Ã¨ uguale
											// a quello cercato
											if ($id != $item) {
												if ($newcart != '') {
													$newcart .= ',' . $item;
							...
							if ($cart) {
								...
								// magari presenti in quantitÃ  maggiori di 1 ed evitare duplicati
								$cart = $_SESSION['cart'];
								if ($cart) {
									$items = explode(',', $cart);
									$contents = array();
